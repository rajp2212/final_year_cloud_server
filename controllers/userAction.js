...
export const signupprovider=async (req,res)=>{
    ...
    try { 
        ...
        const hashedPassword= await bcrypt.hash(password,12);
         ...
         const result= await Provider.create({email,total_VM:available_VM,password:hashedPassword,name:brandName,cost,available_VM,network_bandwidth,security_management,flexibility,response_time,cpu_capacity,memory_size,boot_time,scale_up,scale_down,scale_up_time,scale_down_time,max_vm_configure,auto_scaling,storage});
